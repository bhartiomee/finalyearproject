...
INSERT INTO posts(id, title, body, category, user_id) VALUES ("904b7e6e-0f20-4020-af7b-4bb775c02189", "LocalDate and LocalDateTime in a index which runs in EST vs UTC", "I am trying to understand LocalDate and LocalDateTime. Since they do not carry zone info, how does it work for now() on two different time zone.","blog", "53e1deae-31da-4244-99e6-c0777da311b0");
...
INSERT INTO posts(id, title, body, category, user_id) VALUES ("ee7e47e4-6bfd-4f4f-a3ad-39dd5f37ad27", "Flutter: Specify ListTile height", "The issue is The tile here toke around more than half of the page which makes it looks inconsistent. I want to limit the height of the tile, I have tried putting them in a row and a container and it doesn't work. Any HELP will be appreciated.", "blog", "202598e5-b90b-4002-9def-8d6b496c028a");
SET @v1 := (SELECT LAST_INSERT_ID());
...
INSERT INTO posts(id, title, body,category, user_id) VALUES ("bfc3f286-06e4-40c6-bd9a-220376b5fca8", "Why is processing a sorted array faster than processing an unsorted array?", "Here is a piece of C++ code that shows some very peculiar behavior. For some strange reason, sorting the data miraculously makes the code almost six times faster:","blog" "6963eed1-0ae8-4b13-ba33-065a56c3ccef");
SET @v1 := (SELECT LAST_INSERT_ID());
INSERT IGNORE INTO tags(tagname, description) VALUES ("java", "Java is a popular high-level programming language. Use this tag when you&#39;re having problems using or understanding the language itself. This tag is rarely used alone and is most often used in conjunction with [spring], [spring-boot], [jakarta-ee], [android], [javafx], [gradle] and [maven].");
SET @v2 := (SELECT id FROM tags WHERE tagname = "java");
INSERT INTO posttag(post_id, tag_id) VALUES(@v1, @v2);

INSERT INTO posts(id, title, body,category, user_id) VALUES ("17d97d9b-7c3f-484a-aa48-0a8bf3e2c71c", "Is there a unique Android device ID?", "Do Android devices have a unique ID, and if so, what is a simple way to access it using Java?","blog", "09c1a199-df9c-48f2-aa01-28511b19da36");
SET @v1 := (SELECT LAST_INSERT_ID());
